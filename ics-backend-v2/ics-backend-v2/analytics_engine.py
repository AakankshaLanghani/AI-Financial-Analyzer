...
def _execute_grouped(
    ...
    # Use as_index=False to avoid ValueError when group_col name collides with index
    ...
    return AnalyticsResult(
        ...
        row_count    = len(df),
        groups       = groups,
        formula      = f"SUM({metric_col}) grouped by {group_col}",
...
def _execute_max_min(
    ...
    natural_dimensions = ["product", "sales_person", "region", "category",
                          "department", "customer"]

...
const Field = ({ label, value, onChange, show, onToggle, placeholder, type = 'text', disabled }) => (
  <div>
    ...
    <div style={{ position: 'relative' }}>
      <input
        ...
        style={{
          ...
          padding: onToggle ? '10px 40px 10px 13px' : '10px 13px',
          fontSize: 13.5, border: '1.5px solid #e2e5ea', borderRadius: 9,
          outline: 'none', color: '#1a1f2e', background: '#fafbfc',
          transition: 'border-color 0.15s, box-shadow 0.15s',
        }}
        onFocus={e => { e.target.style.borderColor = '#C31D27'; e.target.style.boxShadow = '0 0 0 3px rgba(195,29,39,0.08)'; }}
      ...
      {onToggle && (
        <button type="button" onClick={onToggle} tabIndex={-1} style={{
          ...
          background: 'none', border: 'none', cursor: 'pointer', color: '#9aa0ad',
  ...
  return (
    <div onClick={handleClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        {success ? (
          <div style={{
            background: '#f0faf4', border: '1px solid #b7e4c7', borderRadius: 10,
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <Field
              ...
              placeholder="Your current password" disabled={loading || !!success}
            />
            ...
            <Field
              ...
              type="email" placeholder="new@example.com" disabled={loading || !!success}
            />
            ...
            <Field
              ...
              placeholder="Min. 8 characters" disabled={loading || !!success}
            />

            <Field
              label="Confirm New Password"
              value={confirmPassword} onChange={setConfirmPassword}
              placeholder="Re-enter new password" disabled={loading || !!success}
            />

            {error && (
              <div style={{
                background: '#fff5f5', border: '1px solid #fecaca', borderRadius: 8,
                padding: '9px 13px', fontSize: 12.5, color: '#b91c1c',
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
              <button type="button" onClick={handleClose} style={{
                padding: '10px 18px', background: 'none', border: '1.5px solid #e2e5ea',
                borderRadius: 9, color: '#6b7280', fontSize: 13, fontWeight: 500,
                cursor: 'pointer', fontFamily: 'inherit',
              }}>
                Cancel
              </button>
              <button type="submit" disabled={loading} style={{
                flex: 1, padding: '10px', background: loading ? '#f3f4f6' : '#C31D27',
                color: loading ? '#9ca3af' : '#fff', border: 'none', borderRadius: 9,

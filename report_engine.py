...
def _draw_cover_background(canvas, doc):
    ...
    # ICS wordmark inside the band
    canvas.setFillColor(ICS_RED)
    canvas.setFont("Helvetica-Bold", 30)
    canvas.drawCentredString(W / 2, H - 58, "ICS")
    canvas.setFillColor(colors.white)
    canvas.setFont("Helvetica", 12)
    canvas.drawCentredString(W / 2, H - 78, "AI Financial Analyzer")
    ...
    canvas.drawCentredString(
        W / 2, 14,
        f"Confidential  ·  Generated by ICS AI Financial Analyzer  ·  "
        f"{datetime.datetime.now().strftime('%d %B %Y')}",
    )
...
def _sales_breakdown(parsed: dict, S: dict) -> list:
    ...
    if cat_result:
        ...
        buf = _hbar_chart(labels, values, "Sales by Product (Top 8)",

...
_METRIC_PATTERNS: Dict[str, List[str]] = {
    ...
    # ── ICS Master Data metrics ──────────────────────────────────────────────
    "payment_days":     ["avg payment days", "average payment days",
                         "payment days", "days to pay", "avg days",
                         "average days", "payment day", "days outstanding"],
    "total_received":   ["total received", "amount received", "cash received",
                         "total collected", "total receipt", "received amount",
                         "collection amount", "collections"],
...
_METRIC_CHECK_ORDER = [
    ...
    "sales_person",
    "payment_days", "total_received", "credit_limit", "credit_term",
]
...
_METRIC_ROUTES: Dict[str, List[str]] = {
    ...
    "cf_net":           ["CASH_FLOW"],
    "payment_days":     ["PRODUCT_SALES"],
    "total_received":   ["PRODUCT_SALES"],
    "credit_limit":     ["PRODUCT_SALES"],
    "credit_term":      ["PRODUCT_SALES"],
}
...
_EXPLICIT_GROUPBY: Dict[str, str] = {
    ...
    "per payment status":  "status",
    "by status":           "status",
    "payment status wise": "status",
...
_WHICH_ENTITY_PATTERNS: List[Tuple[re.Pattern, str]] = [
    ...
    (re.compile(r'\bwhich\s+(?:\w+\s+){0,3}cit(?:y|ies)\b', re.I),  "city"),
    (re.compile(r'\bwhat\s+(?:\w+\s+){0,3}cit(?:y|ies)\b', re.I),   "city"),
...
_TOPN_GROUPBY: List[Tuple[re.Pattern, str]] = [
    ...
    (re.compile(r'\btop\s+\d+\s+cit(?:y|ies)\b',  re.I), "city"),
    (re.compile(r'\bbottom\s+\d+\s+cit(?:y|ies)\b', re.I), "city"),
...
def _detect_group_by(q: str, parsed: dict) -> Optional[str]:
    ...
    _PLURAL_ENTITY: List[Tuple[re.Pattern, str]] = [
        ...
        (re.compile(r'\b(?:all|rank|each|every|list)\s+(?:the\s+)?cit(?:y|ies)\b', re.I), "city"),
        ...
        (re.compile(r'\bpayment\s+status\b', re.I), "status"),
        (re.compile(r'\bby\s+status\b', re.I), "status"),
    ]
    ...
    for sheet in parsed.get("sheets", []):
        ...
        for row in sheet.get("rows", []):
            ...
            for col, val in row.get("data", {}).items():
                ...
                # Pass 2 — prefix/suffix match for NEAR-IDENTICAL singular/plural variants only:
                #   question has "antidiabetic", data has "antidiabetics" → lengths differ by ≤3
                #   This prevents "payment on delivery" (19 chars) matching token "payment" (7 chars)
                prefix_match = False
                if not exact_match and len(vl) >= 4:
                    for tok in re.findall(r"\b[a-z]{4,}\b", q):
                        length_diff = abs(len(vl) - len(tok))
                        if length_diff <= 3 and (vl.startswith(tok) or tok.startswith(vl)):
                            prefix_match = True
                            break
...
def build_query_plan(question: str, parsed: dict) -> QueryPlan:
    ...
    """
    q = question.lower().strip()
    # Normalise common no-space variants so entity detection works
    q = q.replace("ontime", "on time").replace("on-time", "on time")
    ...
    # Force weighted ratio for ALL ratio metrics (GP%, margin%, etc.)
    # margin_pct must ALWAYS be computed as SUM(GP)/SUM(Sale)×100.
    # It must NEVER be summed or averaged from the GP% column directly.
    # Without this, "what is GP%?" with no group_by reads the GP% column and sums it → 823.
    if is_ratio_metric(metric):
        operation = "sum"  # ensures weighted ratio path is taken in analytics engine
